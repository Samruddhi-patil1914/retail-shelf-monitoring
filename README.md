...
```text
       ...
       ?????????????????????????????????
                      ...
                      ?
       ?????????????????????????????????
       ?   Preprocessing (OpenCV/PIL)  ?  --> Resize (640x640), Normalization, CLAHE
       ?????????????????????????????????
                      ?
                      ?
       ?????????????????????????????????
       ?    YOLOv8n Object Detector    ?  --> Bounding Boxes, Classes, Confidences
       ?????????????????????????????????
                      ?
                      ?
       ?????????????????????????????????????????????????????????????
       ?                   Shelf Analysis Core                     ?
       ?  ???????????????????????????????????????????????????????? ?
       ?  ? Product Counter ? Empty-Space Gap  ? Misplacement    ? ?
       ?  ? (per category)  ? Occupancy Anal.  ? Classifier      ? ?
       ?  ???????????????????????????????????????????????????????? ?
       ?????????????????????????????????????????????????????????????
                      ?
                      ?
       ?????????????????????????????????
       ?   SQLite Database Logging     ?  --> zones, detections, reports tables
       ?????????????????????????????????
                      ?
                      ?
       ?????????????????????????????????
       ?   Streamlit Web Dashboard     ?  --> Side-by-side CV, Plots, Alerts, Exports
       ?????????????????????????????????
```
